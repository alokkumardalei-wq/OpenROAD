  ...
  for (int layer_num = 0; layer_num < (int) num_layers; layer_num++) {
    ...
    for (auto& tp : getDesign()->getTopBlock()->getTrackPatterns(layer_num)) {
      if ((getDir() == dbTechLayerDir::HORIZONTAL
        ...
        frCoord low_coord = (is_horizontal ? getRouteBox().yMin() : getRouteBox().xMin());
        frCoord high_coord = (is_horizontal ? getRouteBox().yMax() : getRouteBox().xMax());
        int track_num
          ...
          if (ap == nullptr) {
            ...
            if (pa != nullptr) {
              if (pa->getNumAccessPoints() > 0) {
                // use first ap of frMPin's pin access to set pin_coord of iroute
    ...
    for (auto& nbrGuide : nbr_guides) {
      if (nbrGuide->getNet() == net) {
        ...
        if (!nbrGuide->hasRoutes()) {
          ...
          if (psLNum == layer_num - 2) {
            down_via_coord_set.insert((is_horizontal ? nbrBp.x() : nbrBp.y()));
          } else {
            up_via_coord_set.insert((is_horizontal ? nbrBp.x() : nbrBp.y()));
          }
        } else {
          for (auto& connFig : nbrGuide->getRoutes()) {
            if (connFig->typeId() == frcPathSeg) {
              ...
              if (i == 0) {
                min_begin = std::min(min_begin,
                                    (is_horizontal ? nbrSegBegin.x() : nbrSegBegin.y()));
                has_min_begin = true;
              } else {
              ...
              if (psLNum == layer_num - 2) {
                down_via_coord_set.insert(
                    (is_horizontal ? nbrSegBegin.x() : nbrSegBegin.y()));
              } else {
                up_via_coord_set.insert((is_horizontal ? nbrSegBegin.x() : nbrSegBegin.y()));
              }
            }
          }
        }
      ...
      for (auto& [bounds, obj] : result) {
        ...
        switch (type) {
          case frcInstBlockage: {
            ...
            auto bloat_dist = initFixedObjs_calcOBSBloatDistVia(
